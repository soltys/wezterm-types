...
---@alias EventFormatWindowTitle fun(event: "format-window-title", callback: fun(window: Window, pane: Pane, tabs: MuxTabObj[], panes: Pane[], config: Config): string) When the text for a tab title needs to be recomputed; this event is a bit special in that it is synchronous and must return as quickly as possible in order to avoid blocking the GUI thread.
